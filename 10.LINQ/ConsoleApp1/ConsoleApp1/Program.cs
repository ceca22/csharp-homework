    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            int numberOfSongsStartingWithA = Songs
                                            .Where(x => x.Name.StartsWith("A") || x.Name.EndsWith("a"))
                                            .Count();

            Console.WriteLine($"Songs start with the letter 'a': { numberOfSongsStartingWithA}");
            Console.WriteLine("----------------------------");
            ...
            int artistEndWithA = Artists
                                .Where(x => x.FullName.EndsWith("a") || x.FullName.EndsWith("A"))
                                .Count();

            Console.WriteLine($"Artists end with letter 'a': {artistEndWithA}");

            Console.WriteLine("----------------------------");

            // - whats the name of the song with longest duration
            
            Song longestSongFromAll = Songs
                                    .Where(x => x.Duration == Songs.Select(y => y.Duration).Max())
                                    .SingleOrDefault();

            Console.WriteLine($"Song with longest duration: {longestSongFromAll.Name}");
            Console.WriteLine("----------------------------");
            ...
            Console.WriteLine($"Total Duration of all Songs: {totalDuration} seconds");
            ...
            int albums = Albums
                        .Select(x => x.Songs.Where(y => y.Duration > 300)).ToList().Count();
                                

            Console.WriteLine($"Albums with songs longer than 300sec: {albums}");
            ...
            List<int> artistWithMoreThanOneAlbumPopRock = Artists
                                                    .Where(x => x.Albums.Count > 1)
                                                    .SelectMany(a => a.Albums)
                                                    .Where(a => a.Genre == Genre.PopRock)
                                                    .Select(b => b.ArtistId)
                                                    .ToList();

            Console.WriteLine("-------------names of the artists that have more than one album of PopRock genre--------------");

            artistWithMoreThanOneAlbumPopRock.ForEach(x =>
           Artists.Where(y => y.Id == x).ToList().ForEach(z => Console.WriteLine(z.FullName))
